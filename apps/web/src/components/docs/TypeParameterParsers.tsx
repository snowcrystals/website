...
export function getTypeParametersJsx(anyType: TypeParser.Json, pkg: string, version: string): React.ReactNode {
	switch (anyType.kind) {
		case TypeParser.Kind.Reference: {
			const type = anyType as ReferenceTypeParser.Json;
			const args = type.typeArguments.map((type) => getTypeParametersJsx(type, pkg, version));
		...
		case TypeParser.Kind.Array: {
			const type = anyType as ArrayTypeParser.Json;
			const value = getTypeParametersJsx(type.type, pkg, version);
		...
		case TypeParser.Kind.Conditional: {
			const type = anyType as ConditionalTypeParser.Json;
			const checkType = getTypeParametersJsx(type.checkType, pkg, version);
			const extendsType = getTypeParametersJsx(type.extendsType, pkg, version);
			const trueType = getTypeParametersJsx(type.trueType, pkg, version);
			const falseType = getTypeParametersJsx(type.falseType, pkg, version);
		...
		case TypeParser.Kind.Union: {
			const type = anyType as UnionTypeParser.Json;
			return <span>{type.types.map((t) => getTypeParametersJsx(t, pkg, version)).reduce((prev, curr) => [prev, " | ", curr])}</span>;
		}
		case TypeParser.Kind.Rest: {
			const type = anyType as RestTypeParser.Json;
			const restType = getTypeParametersJsx(type.type, pkg, version);
		...
		case TypeParser.Kind.IndexedAccess: {
			const type = anyType as IndexedAccessTypeParser.Json;
			const objectType = getTypeParametersJsx(type.objectType, pkg, version);
			const objectTypeStr = getTypeParametersString(type.objectType);
			const indexType = getTypeParametersJsx(type.indexType, pkg, version);
		...
		case TypeParser.Kind.Optional: {
			const type = anyType as OptionalTypeParser.Json;
			const optionalType = getTypeParametersJsx(type.type, pkg, version);

			return <span>{optionalType} | undefined</span>;
		}
		case TypeParser.Kind.Intersection: {
			const type = anyType as IntersectionTypeParser.Json;
			return <span>{type.types.map((type) => getTypeParametersJsx(type, pkg, version)).reduce((prev, curr) => [prev, " & ", curr])}</span>;
		}
		case TypeParser.Kind.NamedTupleMember: {
			const type = anyType as NamedTupleMemberTypeParser.Json;
			return (
		...
		case TypeParser.Kind.Predicate: {
			...
			const predicateType = getTypeParametersJsx(type.type, pkg, version);
			return (
				<span>
		...
		case TypeParser.Kind.Query: {
			const type = anyType as QueryTypeParser.Json;
			const queryType = getTypeParametersJsx(type.query, pkg, version);
		...
		case TypeParser.Kind.TemplateLiteral: {
			...
			return (
				<span>
					`
					{type.tail.map((tail, key) => (
						<span key={key}>
							{tail.text}
							{"${"}
							{getTypeParametersJsx(tail.type, pkg, version)}
							{"}"}
						</span>
					))}
					`
		...
		case TypeParser.Kind.Mapped: {
			const type = anyType as MappedTypeParser.Json;
			const parameterType = getTypeParametersJsx(type.parameterType, pkg, version);
			const nameType = type.nameType ? getTypeParametersJsx(type.nameType, pkg, version) : "";
			const templateType = getTypeParametersJsx(type.templateType, pkg, version);
			const isReadonly = type.readonly === "+" ? true : false;
			const isOptional = type.optional === "+" ? true : false;
		...
		case TypeParser.Kind.Reflection: {
			...
			if (properties) {
				const mappedProperties = properties
					.map((property, key) => (
						<span key={key}>
							{property.name}: {getTypeParametersJsx(property.type, pkg, version)}
						</span>
					))
			...
			if (signatures) {
				const mappedSignatures = signatures.map((signature, key) => {
					const name = signature.name === "__type" ? "" : signature.name;
					const params = signature.parameters.map((param, key) => (
						<span key={key}>
							{param.rest ? "..." : ""}
							{param.name}
							{param.optional ? "?" : ""}: {getTypeParametersJsx(param.type, pkg, version)}
						</span>
					));
					const typeParams = signature.typeParameters.map((param, key) => (
						<span key={key}>
							{param.name}
							{param.constraint ? <span>extends ${getTypeParametersJsx(param.constraint, pkg, version)}</span> : ""}
							{param.default ? <span>= {getTypeParametersJsx(param.default, pkg, version)}</span> : ""}
						</span>
					));
					...
					return (
						<span key={key}>
							...
							{getTypeParametersJsx(signature.returnType, pkg, version)}
						</span>
					);
				});
		...
		case TypeParser.Kind.TypeOperator: {
			const type = anyType as TypeOperatorTypeParser.Json;
			const parsedType = getTypeParametersJsx(type.type, pkg, version);
